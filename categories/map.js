 ...
 * map.js
 * ============================================================
 * God's Gift Safaris — Interactive Uganda Map Module
 *
...
async function initMap() {
  ...
  try {
    ...
    attractions = payload.data.attractions;

    if (typeof hidePreloader === 'function') hidePreloader();
  } catch (err) {
    console.error('Failed to load attraction data for map:', err);
    if (typeof hidePreloader === 'function') hidePreloader();
    return;
  }
  ...
  attractions.forEach(attraction => {
    ...
    const popupHTML = `
      <div class="map-popup">
        <img
          src="${window.CloudinaryMedia ? window.CloudinaryMedia.getOptimizedUrl(attraction.images[0], { width: 540, crop: 'fill', gravity: 'auto' }) : attraction.images[0]}"
          alt="${attraction.name}"
          style="
            width: 100%;
            height: 120px;
  ...
  console.log(`✅ God's Gift Safaris map initialized with ${attractions.length} markers.`);
}
